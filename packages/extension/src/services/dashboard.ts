import { manuscriptUploadStatusColumn } from "../content.js";

export const renderDashboard = async () => {
  const dashboardContainer = document.querySelector("#navigationDIV");
  ...
  header.textContent = "Coauthor Dashboard";
...
const renderLoginInterface = () => {
  const dashboard = document.querySelector("#coauthor-dashboard");
  ...
  loginStatusTextWrapper.append(loginStatusText);
  loginSection.append(loginStatusTextWrapper);
  ...
  loginSection.append(logButton);
  dashboard?.append(loginSection);
  ...
  chrome.runtime.sendMessage({ message: "checkAuthStatus" }, (response) => {
